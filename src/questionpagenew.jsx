...
function Test() {
  ...
  const fetchSectionData = async (section) => {
    try {
      const response = await axios.post(`http://localhost:3000/viewpaper${section === 'A' ? '' : 'sectionB'}`, { section });
  ...
  const renderQuestions = () => {
    ...
    return (
      <div style={{ padding: '20px', backgroundColor: '#f9f9f9', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
        <h2 style={{ color: '#333', fontSize: '20px' }}>Serial Number: {question.serialNumber}</h2>
        <h3 style={{ color: '#555', fontSize: '18px' }}>{question.question}</h3>
